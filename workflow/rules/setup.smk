...
if units_dup.any():
    raise ValueError(f"duplicated unit description. Please check that your unit IDs ({index}) are unique:\n{units[units_dup]}")
...
samples_pe_no_read = units[units["seq_type"].eq("pe") & ~units["data"].str.contains("{Read}")].data
